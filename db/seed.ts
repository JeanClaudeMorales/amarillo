...
export async function seedDatabase(): Promise<void> {
    ...
    // Get parroquias from Libertador (190) and Campo Elías (184) to assign APs
    const pars = await query<{ id: number }>('SELECT id FROM parroquias WHERE municipio_id IN (190, 184) ORDER BY id LIMIT 7');
    if (pars.length >= 5) {
        const apData = [
